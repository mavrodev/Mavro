...
int64_t GetProofOfWorkReward(int64_t nFees, int nHeight)
{
    ...
    int nInitial = 5000000;
	int nCurrent = nInitial / pow(2, (int)(nHeight/2));

	if (fDebug && GetBoolArg("-printcreation"))
        printf("GetProofOfWorkReward() : create=%s nSubsidy=%"PRId64" nHeight=%u\n", FormatMoney(nSubsidy).c_str(), nSubsidy, nHeight);
	
    return nCurrent * COIN + nFees;
}

const int DAILY_BLOCKCOUNT =  1440 / 4;
// miner's coin stake reward based on coin age spent (coin-days)
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees, int64_t nMoneySupply)
{
	...
	// 0% after 15m total coins
	if (nMoneySupply > 15000000 * COIN)
		nSubsidy = 0;
...
static const int64_t nTargetTimespan = 4 * 15 * 60;
//
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    // printf("Blocktime: %u --- vtx[0].nTime: %u\n", GetBlockTime(),FutureDrift((int64_t)vtx[0].nTime)); // For troubleshooting only, can be removed.
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "MavroCoin Genesis Block";
        CTransaction txNew;
        if (!fTestNet)
        {
            txNew.nTime = 1453827079;
        }
        else
        {
            txNew.nTime    = 1453827089; // Without separate txNew.nTime near block.nTime, CheckBlock fails on coinbase timestamp for TestNet created on 14042015
        }
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        // debug print
        assert(block.hashMerkleRoot == uint256("0x1886643bc4ebd592c791d533940828a98c5ffb2b4ceb79878e3a1136ad3ed30c"));
        block.print();

//		for (unsigned long ii = 160000; ii < UINT64_MAX; ii++) {
// 			block.nNonce = ii;
// 			printf("Ii = %lu\n", ii);
// 			printf("block.GetHash() == %s\n", block.GetHash().ToString().c_str());
// 			if (block.CheckBlock(true, false, false)) {
// 				break;
// 			}
//		}
...
unsigned char pchMessageStart[4] = { 0x42, 0xdd, 0xe4, 0xe3 };
